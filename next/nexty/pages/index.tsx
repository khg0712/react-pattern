...
export default function Lotation() {
	const { items, selected, positions, phase, onLeft, onRight } = useLotation(3);
	...
	return (
		<div className={style.wrapper}>
			...
			<LotationWrapper
				...
				phase={phase}
			/>
			<button className={`${style.button} ${style.left}`} onClick={onLeft}>
				left
			</button>
			<button className={`${style.button} ${style.right}`} onClick={onRight}>
				right
			</button>
		</div>
	);
